...
1. 启动服务后，在浏览器打开页面。
2. 在左侧输入框里输入一句自然语言需求，例如：帮我把 parser 改成支持更多字段，输出 patch，尽量少改。
3. 点击“预览解析”，先查看系统抽取出的结构化字段，确认 task、language、context、constraints 等信息是否正确。
4. 如果预览无误，点击“开始执行”，右侧会显示模型生成结果。
5. 如果开启了自检，可以在“继续修正说明”里补充新的约束，再点击“继续修正”，系统会沿着上一轮状态继续生成。
6. 最终你可以在“结果工作区”里同时看到对话记录、生成输出和结构化预览，便于检查每一轮请求是怎么被处理的。
